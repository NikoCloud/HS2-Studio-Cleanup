...
def parse_card_gender(path_str):
    ...
    # Read marker string (7-bit encoded int length prefix)
    ...
    # count is number of bytes for blockheader. We can skip parsing and just regex it,
    # or skip count bytes, but wait we need the position of Parameter!
    
    blockHeaderBytes = data[offset:offset+count]
    offset += count
    
    # After blockheader, there is a num (Int64) and then the actual blocks
    num, offset = struct.unpack('<q', data[offset:offset+8])[0], offset+8
    base_pos = offset
    
    # Now find the Parameter block info inside blockHeaderBytes manually
    p_idx = blockHeaderBytes.find(b'\xa9Parameter')
    if p_idx == -1: return None
    
    # Find position of 'pos' key which is b'\xa3pos'
    pos_key_idx = blockHeaderBytes.find(b'\xa3pos', p_idx - 64)
    # Actually 'pos' can be before or after 'name'. Alphabetically 'name' < 'pos' so pos comes after.
    ...
    # Read the parameter block! It's an array: [version, sex, fullname, personality, ...]
